...
The public repository includes `keys/logialog-ed25519-public.pem` and the detached `advisories/snapshot-manifest.sig.json`. Their production key ID is `4b563811547e5518`; CI verifies the signature on every change. Manifest signing normalizes CRLF to LF so the same tracked JSON verifies on Windows and Linux. The encrypted private key and its Windows DPAPI-protected recovery secret are stored outside the checkout and restricted to the owner account.
